...
import { publishSourceDiagnostics } from './source-diagnostics'
...
export const SOURCE_TIMEOUTS_MS: Record<string, number> = {
  // GitHub repository-contributor discovery can perform several bounded public requests.
  github: 12000,
  npm: 5000, pypi: 5000, crates: 5000, rubygems: 5000,
 ...
 * A parent cancellation is distinguishishable from a source timeout so stale
...
export async function fetchWithTimeout(
  ...
  try {
    ...
    const data = await response.json() as { diagnostics?: unknown; warnings?: unknown }
