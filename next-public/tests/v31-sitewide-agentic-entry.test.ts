...
describe('V37 canonical sourcing entry points', () => {
  ...
  it('promotes one authenticated Search workflow instead of three competing sourcing destinations', () => {
    const shell = source('components/AppShell.tsx')
    expect(shell).toContain("{ href: '/app/search', label: 'Search'")
    expect(shell).toContain("{ href: '/app/candidate-database', label: 'Talent'")
    expect(shell).toContain("{ href: '/app/sources', label: 'Sources'")
    expect(shell).not.toContain("label: 'People Search'")
    expect(shell).not.toContain("label: 'AI Sourcing'")
    expect(shell).not.toContain("label: 'Talent Insights'")
  })

  it('keeps the canonical Search, Talent, and Sources workflows directly available in the global command palette', () => {
    ...
    expect(palette).toContain("label: 'Search'")
  })
