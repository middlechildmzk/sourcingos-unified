...
describe('V27 Today decision inbox', () => {
  ...
  it('routes the app root and duplicate Agent OS route to the single Today surface', () => {
    ...
    expect(shell).not.toContain("{ href: '/app/agent-os', label: 'Ask SourcingOS'")
    ...
    expect(client).toContain('SourcingOS prioritizes the work; you make the call.')
    expect(client).toContain('Autonomous research · Human hiring decisions')
  })
})
