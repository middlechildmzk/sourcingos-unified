...
describe('V36.8 Candidate Data Fabric', () => {
  it('keeps Pearch search contact reveal off by default and preserves structured requirements', () => {
    const body = buildPearchSearchBodyV36_8(request)
    expect(body).toMatchObject({ type: 'fast', insights: false, profile_scoring: true, reveal_emails: false, reveal_phones: false, high_freshness: false })
    expect(body).toHaveProperty('search_requirements')
    ...
    const adapters: CandidateDataSearchAdapterV36_8[] = [
      {
        provider: 'pearch',
        search: async () => ({
          observations: [1, 2, 3].map(index => ({ provider: 'pearch' as const, providerPersonId: `p${index}`, displayName: `P ${index}`, skills: [], profileUrls: [], contactAvailability: { email: 'unknown' as const, phone: 'unknown' as const }, observedAt: '2026-09-02T20:00:00.000Z' })),
          telemetry: { provider: 'pearch', status: 'completed', discovered: 3, latencyMs: 1 }, warnings: [],
        }),
      },
      {
        provider: 'data_vertex',
        search: async () => ({
          observations: [1, 2, 3].map(index => ({ provider: 'data_vertex' as const, providerPersonId: `d${index}`, displayName: `D ${index}`, skills: [], profileUrls: [], contactAvailability: { email: 'unknown' as const, phone: 'unknown' as const }, observedAt: '2026-09-02T20:00:00.000Z' })),
          telemetry: { provider: 'data_vertex', status: 'completed', discovered: 3, latencyMs: 1 }, warnings: [],
        }),
      },
    ]
  ...
  it('requires a deterministic DataVertex lookup anchor and keeps enrichment explicit', () => {
    ...
    expect(buildDataVertexLookupBodyV36_8({ providerName: 'data_vertex', providerPersonId: '12345' }, 'work_email_finder')).toEqual({
      candidate_id: '12345',
