...
function strategyOutput(input: RoleLaunchInput) {
...
async function executeStep(ownerId: string, workflow: any, step: any) {
  ...
  if (step.step_key === 'intake') {
    ...
    output = { queue: 'autosource_inbox', policy: 'Rank by role relevance, evidence quality, identity confidence, and recruiter memory. No automated rejection.' }
  } else if (step.step_key === 'calibration') {
    output = { minimumReviewedCandidates: 5, memoryPolicy: 'Only learn patterns supported by repeated recruiter decisions. All inferred signals remain inspectable and reversible.' }
  ...
  if (definition?.approval) {
    const { data: approval, error } = await sb.from('agent_approvals').insert({
      ...
      summary: step.step_key === 'strategy' ? 'Review search intent, connectors, target companies, and guardrails before activation.' : 'Review learned sourcing preferences before future searches use them.',
      payload: output,
    }).select('id').single()
    if (error) throw new Error(error.message)
...
export async function advanceWorkflow(ownerId: string, workflowId: string) {
  ...
  if (!next) {
    const { data } = await sb.from('agent_workflows').update({ status: 'completed', current_step: 'complete', completed_at: now(), updated_at: now() }).eq('id', workflowId).eq('owner_id', ownerId).select('*').single()
...
export async function decideApproval(ownerId: string, approvalId: string, decision: 'approved' | 'rejected', note = '') {
  const sb = createServerSupabaseClient()
  if (!sb) throw new Error('Supabase client unavailable.')
  const { data: approval, error } = await sb.from('agent_approvals').select('*').eq('id', approvalId).eq('owner_id', ownerId).eq('status', 'pending').single()
