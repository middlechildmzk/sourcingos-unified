...
export async function searchPyPi(req: SourceSearchRequest): Promise<SourceResult[]> {
  ...
  for (const term of targets.slice(0, Math.min(req.limit || 5, 5))) {
    try {
      ...
      // Package-name search is intentionally conservative. Return no results if no packages resolve.
    }
  }
  return out.length ? out : []
}
...
export async function searchSources(req: SourceSearchRequest) {
  ...
  const warnings = settled.flatMap((s, i) => s.status === 'rejected' ? [`${selected[i]} failed. The source did not return a usable result.`] : [])
