...
const description = 'A recruiter-first 2026 guide to ContactOut, Lusha, Apollo, Hunter and contact-data workflows, plus a 25-candidate test protocol for coverage, verification, phone quality, freshness, cost, and policy fit.'
...
export const metadata: Metadata = {
  ...
  openGraph: { title, description, type:'article', url:canonical, publishedTime:'2026-08-18', modifiedTime:'2026-08-20', authors:['Dan Larson'] },
...
const tools = [
  ...
  },
  {
    ...
    fit:'Lusha explicitly markets recruiting workflows that combine candidate search, contact details, browser-extension lookup, AI recommendations, outreach sequences, and integrations.',
    verify:'Test whether its recruiting search adds evidence-fit leads beyond your existing sources, then separately score contact coverage and the value of its automation for your team.',
  ...
  },
  {
    ...
    verify:'Use the same candidate benchmark as the recruiter-oriented tools. Track successful email/phone reveals, stale employer/title records, recruiter-market coverage, and correction time.',
  ...
  },
  {
    ...
    verify:'Score work-email hit rate and source transparency. Do not penalize it for lacking the same candidate-search breadth as a full prospect database; that is a different job.',
...
const protocol = [
  ...
  ['Measure email coverage','Track work email found, personal email found, no result, provider verification status, and whether the provider exposes source or confidence information.'],
  ...
  ['Calculate usable cost','Divide actual monthly spend or consumed credits by recruiter-confirmed usable contacts, not by vendor database size or theoretical credit allotment.'],
...
const faq = [
  ...
  ['Should enrichment happen before fit review?', 'Usually no. Confirm that the person is relevant enough to justify enrichment, confirm identity as needed, and then use only contact sources and channels your organization is authorized to use.'],
  ['Does finding a contact method mean I can automate outreach?', 'No. Contact availability, lawful use, employer policy, relevance, channel choice, frequency, and opt-out handling are separate questions. SourcingOS treats enrichment and outreach as gated decisions rather than automatic consequences of finding a record.'],
] as const
...
export default function ContactFinderGuide(){
  ...
  const articleSchema = {
    ...
    datePublished:'2026-08-18',dateModified:'2026-08-20',author:{'@type':'Person',name:'Dan Larson',url:`${siteUrl}/about/`},
  ...
  return <>
    ...
    <main className="wrap article article-pro">
      <div className="article-hero-card">
        ...
        <h1>{title}</h1>
        <p className="muted" style={{fontSize:13,margin:'4px 0 12px'}}>Dan Larson · Senior Technical Sourcer · Published August 18, 2026 · Updated August 20, 2026</p>
      ...
      <div className="article-layout">
        ...
        <article className="article-main">
          ...
          <section id="gate"><h2>Put a decision gate before contact enrichment</h2><p>Contact enrichment should not be the first automatic step after a name appears in search. Use a simple gate:</p><ol><li><strong>Relevance first.</strong> Is there enough job-relevant evidence to justify additional contact-data processing?</li><li><strong>Identity next.</strong> Are you confident the contact record belongs to the intended person? If not, resolve the identity question before outreach.</li><li><strong>Source permission.</strong> Is your team authorized to use the data source and the returned channel?</li><li><strong>Channel choice.</strong> Is professional email, platform-native messaging, phone, or another channel appropriate for this context?</li><li><strong>Outreach control.</strong> Keep relevance, personalization, frequency, opt-out handling, and any automation inside the organization&apos;s approved process.</li></ol><p>The existence of a contact record does not erase these decisions. Treat enrichment as a gated workflow, not a spam accelerator.</p></section>
