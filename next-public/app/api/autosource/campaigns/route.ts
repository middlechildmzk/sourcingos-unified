...
import { runCampaign } from '@/lib/acquisition-engine-v22'
...
export async function GET(req: NextRequest) {
  ...
  if (!isSupabaseConfigured() || gate.preview) return NextResponse.json({ ok: true, mode: 'preview', campaigns: [], runs: [], inbox: [] })
  const sb = createServerSupabaseClient(); if (!sb) return NextResponse.json({ ok: false, error: 'Supabase client unavailable.' }, { status: 500 })
  const [campaigns, runs, inbox, review] = await Promise.all([
  ...
  const error = campaigns.error || runs.error || inbox.error || review.error
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
  return NextResponse.json({ ok: true, mode: 'supabase', campaigns: campaigns.data || [], runs: runs.data || [], inbox: inbox.data || [], review: review.data || [] })
}
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'review') {
    ...
    const { error } = await sb.from('acquisition_discoveries').update({ disposition: decision, review_reason: String(body.reason || '').slice(0, 500) }).eq('id', discoveryId).eq('owner_id', gate.userId).eq('disposition', 'needs_review')
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 })
    return NextResponse.json({ ok: true })
  }
