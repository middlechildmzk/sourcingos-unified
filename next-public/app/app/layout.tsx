    ...
    <>
        ...
        <div className="app-session-inner">
          ...
          <div className="app-session-user">
            ...
            {session.authenticated && session.user && (
              <>
                {session.user.role === 'admin' && <Link href="/jobs/admin" className="status-live" style={{ marginRight: '8px', textDecoration: 'none' }}>Admin</Link>}
                <span title={session.user.email} style={{ fontSize: '12px', color: 'var(--muted)', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'inline-block', verticalAlign: 'middle', marginRight: '10px' }}>{session.user.email}</span>
      ...
      {session.mode === 'preview' && (
        <div style={{ background: 'rgba(246,201,107,.04)', borderBottom: '1px solid rgba(246,201,107,.18)', padding: '10px 7vw', fontSize: '12px', color: '#c8a84b', display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span>◈</span>
          <span><strong>Preview mode:</strong> Auth and server persistence are inactive. V20 Role Workspaces use browser-local storage so you can test the full flow without uploading candidate data to a shared preview database.</span>
        </div>
      )}
