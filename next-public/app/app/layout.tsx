...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return <>
    ...
    {session.mode === 'preview' && <div style={{ background: 'rgba(246,201,107,.04)', borderBottom: '1px solid rgba(246,201,107,.18)', padding: '10px 7vw', fontSize: '12px', color: '#c8a84b', display: 'flex', gap: '8px', alignItems: 'center' }}><span>◈</span><span><strong>Preview mode:</strong> Auth and server persistence are inactive. Role Workspaces use browser-local storage; autonomous acquisition requires authenticated durable storage.</span></div>}
    {children}
  </>
}
