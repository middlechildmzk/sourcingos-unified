...
export function RoleAgenticSearchPanel({ roleId }: { roleId: string }) {
  ...
  const plan = useMemo(
    () => role ? buildCanonicalAgenticSearchPlan(role.intake, role.calibration) : null,
    [role],
  )
  ...
  useEffect(() => {
    if (plan && !plan.lanes.some(lane => lane.id === laneId)) {
      setLaneId(plan.lanes[0]?.id || 'exact_title')
    }
  }, [laneId, plan])
  ...
  const waiting = role.candidates.filter(candidate => candidate.fitDecision === 'unreviewed').length

  function saveAttempts(next: SearchAttempt[]) {
    const trimmed = next.slice(-100)
    setAttempts(trimmed)
    try {
      localStorage.setItem(memoryKey(roleId), JSON.stringify(trimmed))
    } catch {
      // Search memory is best-effort in the browser.
    }
  }
  ...
  async function runPublicSources() {
    ...
    try {
      ...
      const completed = running.map(attempt => {
        const connectorSet = attempt.surface === 'github'
          ? new Set<AgenticConnectorKey>(['github'])
          : RESEARCH_CONNECTORS
        const keys = found
          .filter(item => connectorSet.has(item.sourceKey))
          .map(item => `${item.sourceKey}:${item.sourceId}`)
        const statuses = Array.from(connectorSet)
          .map(key => json.sourceStatus?.[key]?.status)
          .filter(Boolean)
        ...
        return {
          ...
          resultKeys: keys,
          completedAt,
          message: `${keys.length} source identities returned.`,
        }
      })
  ...
  return (
    <section className="agentic-search-panel" aria-label="Role research strategy">
      <div className="agentic-spine" aria-label="Sourcing loop">
        <span><b>Brief</b><small>{role.intake.mustHaves.length} must-haves</small></span><i>→</i>
        <span className="active"><b>Strategy</b><small>{plan.lanes.length} hypotheses · v{plan.revision}</small></span><i>→</i>
        <span><b>Slate</b><small>{role.candidates.length} people</small></span><i>→</i>
      ...
      <div className="agentic-search-head">
        <div>
          <span className="kicker">Role research strategy</span>
          <h2>Distinct hypotheses with visible source truth.</h2>
          <p>Each lane states why it exists, what it can miss, and which sources SourcingOS can execute versus merely guide.</p>
        </div>
        <div className="agentic-integrity">
          <b>{plan.distinctQueryCount}/{plan.lanes.length}</b>
          <span>distinct strategy queries</span>
          <small>{taskDistinctness.distinctCount}/{taskDistinctness.taskCount} executable task fingerprints</small>
        </div>
      </div>

      {!!plan.integrityWarnings.length && (
        <div className="agentic-warning-list">
          {plan.integrityWarnings.map(warning => <span key={warning}>⚠ {warning}</span>)}
        </div>
      )}
      ...
      <div className="agentic-lane-tabs" role="tablist" aria-label="Research hypotheses">
        {plan.lanes.map(item => (
          <button
            key={item.id}
            role="tab"
            aria-selected={item.id === lane.id}
            className={item.id === lane.id ? 'active' : ''}
            onClick={() => {
              setLaneId(item.id)
              setResults([])
              setSourceStatus({})
          ...
          >
            <b>{item.priority}</b><span>{item.label}</span>
          </button>
        ))}
      </div>
      ...
      <div className="agentic-plan-grid">
        <div className="agentic-lane-detail">
          ...
          <div className="agentic-source-truth-head">
            <b>Source tasks</b>
            <small>{truth.executable || 0} executable · {truth.guided || 0} guided · {truth.provider_optional || 0} provider-ready</small>
          </div>
          <div className="agentic-source-task-list">
            {lane.tasks.map(task => (
              <div className="agentic-source-task" key={`${lane.id}:${task.surface}`}>
                <div>
                  <span className={`agentic-mode-dot ${task.mode}`} />
                  <div><b>{task.label}</b><small>{task.truth}</small></div>
                </div>
                <span className={`status-pill ${task.mode === 'executable' ? 'success' : task.mode === 'guided' ? 'active' : ''}`}>
                  {task.mode.replace('_', ' ')}
                </span>
              </div>
            ))}
        ...
        <aside className="agentic-run-card">
          <div>
            <span className="kicker">Research pass</span>
            <h3>Run what SourcingOS can actually access.</h3>
            <p>Read-only public research. Discoveries are not saved, merged, rejected, contacted, or treated as verified.</p>
          </div>
          <div className="agentic-run-metrics">
            <span><b>{coverage.uniqueSearches}</b><small>unique searches</small></span>
            <span><b>{coverage.surfacesSearched}</b><small>surfaces run</small></span>
            <span><b>{coverage.uniqueResultsSeen}</b><small>identities seen</small></span>
          </div>
          <button className="btn" disabled={working || !executableConnectorKeys(lane).length} onClick={() => void runPublicSources()}>
            {working ? 'Researching…' : executableConnectorKeys(lane).length ? 'Run public sources' : 'No executable public source'}
          </button>
          <Link className="btn secondary" href={`/app/candidate-search?roleId=${encodeURIComponent(role.id)}`}>Open Candidate Search</Link>
      ...
      {!!Object.keys(sourceStatus || {}).length && (
        <div className="agentic-source-status-row">
          {Object.entries(sourceStatus || {}).map(([key, value]) => (
            <span key={key} className={`status-pill ${value.status === 'completed' ? 'success' : value.status === 'failed' ? 'warning' : ''}`}>
              {key}: {value.status} · {value.discovered}
            </span>
          ))}
        </div>
      )}
      ...
      {!!results.length && (
        <div className="agentic-results">
          <div className="agentic-results-head">
            <div><span className="kicker">Public-source discoveries</span><h3>{results.length} records to inspect</h3></div>
            {novelty !== null && <span className="status-pill active">{novelty}% novel vs role search memory</span>}
          </div>
          <div className="agentic-result-grid">
            {results.slice(0, 18).map(result => (
              <article className="agentic-result-card" key={`${result.sourceKey}:${result.sourceId}`}>
                ...
                <div className="agentic-result-foot">
                  <span>Identity {result.identityConfidence}</span>
                  <span>Profile {result.profileQuality}</span>
                  {result.sourceUrl && <a href={result.sourceUrl} target="_blank" rel="noreferrer noopener">Source ↗</a>}
                </div>
              </article>
            ))}
          </div>
