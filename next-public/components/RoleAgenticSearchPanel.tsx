...
export function RoleAgenticSearchPanel({ roleId }: { roleId: string }) {
  ...
  async function runPublicSources() {
    ...
    const allowed = executableTasks.filter(task => shouldExecuteSearch(attempts, task.surface, task.query).execute)
    if (!allowed.length) {
      setStatus('Search memory blocked an exact repeat. Change the strategy or role criteria before spending another search.')
      return
    }

    const connectors = Array.from(new Set(allowed.flatMap(task => task.connectorKeys || [])))
    const query = allowed[0].query
    const startedAt = new Date().toISOString()
    const running: SearchAttempt[] = allowed.map(task => ({
  ...
  return <section className="agentic-search-panel" aria-label="Role research strategy">
    ...
    <div className="agentic-lane-tabs" role="tablist" aria-label="Research hypotheses">
      {plan.lanes.map(item => <button key={item.id} role="tab" aria-selected={item.id === lane.id} className={item.id === lane.id ? 'active' : ''} onClick={() => { setLaneId(item.id); setResults([]); setStatus(''); setNovelty(null) }}><b>{item.priority}</b><span>{item.label}</span></button>)}
    </div>
