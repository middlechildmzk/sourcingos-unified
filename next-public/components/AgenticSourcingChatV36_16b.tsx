...
function SearchResults({ result }: { result: SearchResult }) {
  ...
  return <div className="agent-chat-search-result">
    <div className="agent-chat-result-summary"><div><span className="kicker">Search result</span><h3 style={{ margin: '4px 0 0' }}>{retained.length} people returned</h3></div><div className="chips"><span className="status-pill">{result.discoveredBeforeCap || retained.length} discovered</span><span className="status-pill success">{result.returnedAfterCap || retained.length} retained</span><span className="status-pill">{result.contributingProviders || 0} sources</span></div></div>
    <div className="agent-chat-candidates">{visible.map((person, index) => <div className="agent-chat-candidate" key={`${person.provider}:${person.providerPersonId}`}><div className="agent-chat-candidate-rank">{index + 1}</div><div style={{ minWidth: 0, flex: 1 }}><div className="agent-chat-candidate-head"><strong>{person.displayName}</strong><span className="status-pill">{label(person.provider)}</span></div><div className="muted" style={{ fontSize: 12, marginTop: 3 }}>{[person.currentTitle || person.headline, person.currentEmployer, person.location].filter(Boolean).join(' · ') || 'Professional observation'}</div>{!!person.skills?.length && <div className="chips" style={{ marginTop: 7 }}>{person.skills.slice(0, 6).map(skill => <span className="tag" key={skill}>{skill}</span>)}</div>}<div className="agent-chat-contact-state"><span>Email {channel(person.contactAvailability?.email)}</span><span>Phone {channel(person.contactAvailability?.phone)}</span>{(person.profileUrls || []).slice(0, 2).map(profile => <a key={`${profile.kind}:${profile.url}`} href={profile.url} target="_blank" rel="noreferrer">{label(profile.kind)}</a>)}</div></div></div>)}{!retained.length && <div className="product-empty-state"><h3>No people returned</h3><p className="muted">Refine the criteria in the conversation. Provider failures and zero-result searches remain distinct in diagnostics.</p></div>}</div>
    ...
    <details className="advanced-disclosure agent-chat-diagnostics"><summary>Search diagnostics · {(result.telemetry || []).filter(item => item.discovered > 0).length} contributing sources</summary><div className="agent-chat-telemetry">{(result.telemetry || []).map(item => <span className={`status-pill ${item.status === 'completed' ? 'success' : ''}`} key={item.provider}>{label(item.provider)} · {item.status} · {item.discovered}</span>)}</div></details>
  </div>
}
...
function WebResearch({ result }: { result: WebResearchResult }) {
  ...
  return <div className="agent-chat-web-result"><div className="agent-chat-result-summary"><div><span className="kicker">Live web research</span><h3 style={{ margin: '4px 0 0' }}>Fresh external source material</h3></div><div className="chips"><span className="status-pill success">{label(result.provider || 'web')}</span>{result.transport && <span className="status-pill">{result.transport.toUpperCase()}</span>}<span className="status-pill">untrusted evidence input</span></div></div><div className="agent-chat-web-copy">{preview || 'The live-web tool returned no readable text.'}</div>{!!remainder && <details className="advanced-disclosure" style={{ marginTop: 10 }}><summary>Show remaining web result</summary><div className="agent-chat-web-copy" style={{ marginTop: 8 }}>{remainder}</div></details>}<div className="muted" style={{ marginTop: 10, fontSize: 10 }}>Live web material is not automatically candidate truth, verified identity, qualification, or contact permission.</div></div>
}
...
function ContactApproval({ plan, contact, disabled, onApprove }: { plan: PeoplePlan; contact?: ContactBatch; disabled: boolean; onApprove: () => void }) {
  const tool = plan.toolPlan.find(item => item.tool === 'find_contacts')
  if (!tool) return <div className="agent-approval-card"><strong>Approval checkpoint</strong><p className="muted">This action is recognized but is not executable in this release yet.</p></div>
...
export function AgenticSourcingChatV36_16b() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      await Promise.all(jobs.map(async ref => {
        try {
          const response = await fetch(`/api/contact-enrichment/jobs/${encodeURIComponent(ref.jobId)}`, { headers: { accept: 'application/json' }, cache: 'no-store' })
  ...
  async function submit(event?: FormEvent) {
    ...
    setWorking('planning')
    try {
      const planRes = await fetch('/api/agent-runtime/plan', { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json' }, body: JSON.stringify({ message, ...(previousPlan ? { previousPlan } : {}) }) })
      ...
      if (plan.action === 'search_web') {
        setWorking('web')
        const webRes = await fetch('/api/agentic-sourcing/web', { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json' }, body: JSON.stringify(plan.webRequest) })
  ...
  async function approveContacts(turnId: string) {
    ...
    try {
      for (const person of people) {
        const identity = identityPayload(person)
        let outcome: ContactOutcome
        try {
          const response = await fetch('/api/contact-enrichment/find', { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json' }, body: JSON.stringify({ purpose: 'contact_bundle', ...identity }) })
          const json = await response.json().catch(() => ({}))
          const signals: ContactSignal[] = Array.isArray(json.signals) ? json.signals : []
          const missingGoals: string[] = Array.isArray(json.orchestration?.missingGoals) ? json.orchestration.missingGoals : []
          outcome = { person, ok: Boolean(response.ok && json.ok), message: String(json.message || json.error || 'Contact enrichment completed.'), signals, missingGoals, ...(!response.ok || !json.ok ? { error: String(json.error || 'Synchronous contact enrichment failed.') } : {}) }
