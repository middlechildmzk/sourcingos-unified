...
const presets = [
  ...
  { label: 'RN Healthcare', query: 'Registered Nurse ICU', location: 'Minneapolis', sources: sourceGroups.healthcare },
...
export function SourceSearchClient() {
  ...
  async function refreshCandidate(profile: CandidateGraphProfile) {
    ...
    try {
      ...
      const json = await res.json()
      if (json.ok) setNotice(`Refreshed ${json.candidate.canonicalName}`)
      else setNotice(json.error || 'Refresh failed')
      await refreshQueue()
    } finally {
      setLoading(false)
    }
  ...
  async function saveGraph() {
    ...
    const json = await res.json()
    if (json.ok) setNotice(`Saved ${json.savedCount} candidate graph profile(s) to the preview store.`)
  ...
  async function scheduledRefresh() {
    ...
    try {
      ...
      const json = await res.json()
      if (json.ok) setNotice(`Scheduled refresh checked due queue. Refreshed ${json.refreshed.length} candidate(s).`)
      else setNotice(json.error || 'Scheduled refresh failed')
      await refreshQueue()
    } finally {
      setLoading(false)
    }
  ...
  async function decideMerge(profile: CandidateGraphProfile, decision: 'confirmed' | 'rejected') {
    ...
    const json = await res.json()
    setNotice(json.ok ? `${decision} merge decision saved for ${profile.canonicalName}` : json.error || 'Merge decision failed')
    await refreshQueue()
  }
  ...
  function exportGraph() {
    ...
    setNotice('Candidate graph JSON copied to clipboard')
  }
  ...
  return <div className="interactive-tool">
    <div className="grid two">
      ...
      <div>
        ...
        <input className="input" value={location} onChange={e => setLocation(e.target.value)} placeholder="Virginia, Minneapolis, Remote" />
      </div>
    </div>
    ...
    <div className="button-row">
      <button className="btn" onClick={runSearch} disabled={loading}>{loading ? 'Working...' : 'Search connected sources'}</button>
      <button className="btn secondary" onClick={scheduledRefresh} disabled={loading}>Run scheduled refresh preview</button>
    </div>
    {notice ? <div className="cta"><b>{notice}</b></div> : null}
    {data?.warnings?.length ? <div className="cta"><b>Warnings</b><ul>{data.warnings.map(w => <li key={w}>{w}</li>)}</ul></div> : null}
    {data?.guardrails?.length ? <div className="cta"><b>Connector guardrails</b><ul>{data.guardrails.map(g => <li key={g}>{g}</li>)}</ul></div> : null}

    {data?.results?.length ? <section><h2>Source profiles</h2><p className="muted">Select profiles to compare identity signals. SourcingOS never auto-merges candidates.</p><div className="results">{data.results.map(r => <div className="result-card" key={r.id}>
    ...
    {pairScores.length ? <section><h2>Identity match review</h2><div className="results">{pairScores.map(p => <div className="result-card" key={`${p.a.id}-${p.b.id}`}><div className="result-head"><span>{p.status}</span><span>{p.score}/100</span></div><h3>{p.a.displayName} ↔ {p.b.displayName}</h3><p className="muted">Recruiter must confirm before merge.</p><ul>{p.reasons.map(r => <li key={r}>{r}</li>)}</ul></div>)}</div></section> : null}

    {data?.candidateGraph?.length ? <section><h2>Candidate graph preview</h2><p className="muted">Source profiles roll up into a candidate graph with match reviews, refresh policies, and a next refresh timestamp. Persistence is preview-backed now and Supabase-ready through the included SQL schema.</p><div className="results">{data.candidateGraph.map(c => <div className="result-card" key={c.id}><div className="result-head"><span>{c.status}</span><span>{c.matchScore}/100 max match</span></div><h3>{c.canonicalName}</h3><p className="muted">{c.sourceProfiles.length} source profile(s), {c.evidenceCount} evidence items, {c.contactSignalCount} contact signals.</p><p><b>Next refresh:</b> {new Date(c.nextRefreshAt).toLocaleString()}</p><ul>{c.sourceProfiles.map(sp => <li key={sp.id}>{sourceLabels[sp.source]}: {sp.profileUrl || sp.sourceProfileId}</li>)}</ul></div>)}</div><div className="button-row"><button className="btn" onClick={saveGraph}>Save candidate graph</button><button className="btn secondary" onClick={exportGraph}>Copy candidate graph JSON</button></div></section> : null}

    <section>
      <h2>Persistent graph preview store</h2>
      <p className="muted">This preview store simulates the database layer. In production it maps to candidates, source_profiles, identity_match_reviews, and candidate_refresh_events.</p>
      {queue?.candidates?.length ? <div className="results">{queue.candidates.map(c => <div className="result-card" key={c.id}><div className="result-head"><span>{c.status}</span><span>{c.sourceProfiles.length} source(s)</span></div><h3>{c.canonicalName}</h3><p className="muted">Last refreshed {new Date(c.lastRefreshedAt).toLocaleString()} · next refresh {new Date(c.nextRefreshAt).toLocaleString()}</p><div className="button-row"><button onClick={() => refreshCandidate(c)}>Refresh now</button><button onClick={() => decideMerge(c, 'confirmed')}>Confirm merge</button><button onClick={() => decideMerge(c, 'rejected')}>Reject merge</button></div></div>)}</div> : <p className="muted">No saved candidates yet. Search sources, build a candidate graph, then save it.</p>}
      {queue?.events?.length ? <div className="cta"><b>Recent graph events</b><ul>{queue.events.slice(0, 6).map(e => <li key={e.id}>{e.type}: {e.detail}</li>)}</ul></div> : null}
    </section>
  </div>
}
