...
## Lesson — failing tests can reveal real engine gaps, not just bad assertions (2026-06-12)
**When a deterministic-helper test fails, check whether the engine is actually wrong before adjusting the test — the search-assist failures surfaced two genuine gaps.**
Writing the five spec QA cases first exposed that (1) cleared source-lane suggestions were being crowded out by the result cap on dense cleared queries, and (2) bare clearance words like "Secret" and non-taxonomy markets like "Huntsville" produced no clearance caution note or cleared-lane suggestion. Both were fixed in lib/search-assist.ts (raw-hint clearance detection + lane rank above exclusions + cap raised to 28), not papered over in the test. Only one assertion was adjusted, and only because it tested raw entity recognition the taxonomy genuinely doesn't do ("Secret" alias is "secret clearance") rather than the functional contract (cleared lane + caution note both fire correctly).
