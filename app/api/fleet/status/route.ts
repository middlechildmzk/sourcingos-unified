...
import { fleetAgentSummaryV40_4, FLEET_AGENTS_V40_4 } from '@/lib/fleet/agent-registry-v40-4'

export const dynamic = 'force-dynamic'

type CountResult = { count: number | null; error: { message: string; code?: string } | null }

async function exactCount(query: PromiseLike<CountResult>, fallback = 0): Promise<number> {
...
export async function GET() {
  ...
  const agentSummary = fleetAgentSummaryV40_4()

  if (gate.preview || !isSupabaseConfigured()) {
    return NextResponse.json({
      ok: true,
      preview: true,
      scheduler: { state: 'preview', discoveryCadenceMinutes: 30, enrichmentCadenceMinutes: 15, activeLanes: 0, pausedLanes: 0, lastRunAt: null, nextDueAt: null },
      ...
      agents: { ...agentSummary, definitions: FLEET_AGENTS_V40_4 },
      lanes: [], sources: [],
    })
  }
  ...
  try {
    const [
      lanesResult, telemetryResult,
      candidates, sourceProfiles, evidenceItems,
      candidates24h, sourceProfiles24h, evidence24h,
      rawDiscoveries24h, pendingIdentityReviews,
      enrichQueued, enrichRunning, enrichNeedsReview, enrichCompleted24h,
      resumeLeads, resumesAttached, profileFacts24h,
    ] = await Promise.all([
      sb.from('fleet_standing_intents')
        ...
        .eq('owner_id', gate.userId).order('created_at', { ascending: true }),
      sb.from('fleet_run_telemetry')
        ...
        .eq('owner_id', gate.userId).gte('started_at', since7d).order('started_at', { ascending: false }).limit(1000),
      ...
      exactCount(sb.from('candidate_enrichment_tasks').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).eq('status', 'queued')),
      exactCount(sb.from('candidate_enrichment_tasks').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).eq('status', 'running')),
      exactCount(sb.from('candidate_enrichment_tasks').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).eq('status', 'needs_review')),
      exactCount(sb.from('candidate_enrichment_tasks').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).eq('status', 'complete').gte('completed_at', since24h)),
      exactCount(sb.from('public_document_leads').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId)),
      exactCount(sb.from('public_document_leads').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).eq('status', 'parsed_attached')),
      exactCount(sb.from('candidate_profile_facts').select('id', { count: 'exact', head: true }).eq('owner_id', gate.userId).gte('created_at', since24h)),
    ])
    ...
    const lanes = (lanesResult.data || []).map(lane => ({
      ...lane,
      next_due_at: lane.enabled && !lane.paused_reason ? nextDueAt(lane.last_run_at, Number(lane.cadence_minutes || 30)) : null,
    }))
    ...
    return NextResponse.json({
      ...
      scheduler: {
        ...
        discoveryCadenceMinutes: 30,
        enrichmentCadenceMinutes: 15,
      ...
      enrichment: { queued: enrichQueued, running: enrichRunning, needsReview: enrichNeedsReview, completed24h: enrichCompleted24h, resumeLeads, resumesAttached, profileFacts24h },
      agents: { ...agentSummary, definitions: FLEET_AGENTS_V40_4 },
      lanes,
      sources: [...sourceMap.values()].sort((a, b) => b.persisted - a.persisted || b.found - a.found),
      trust: { identityMergeAuthorized: false, contactValuesCaptured: false, recruiterDecisionAutomated: false, rawProviderContactValuesPersisted: false, authBypassAllowed: false },
    })
  } catch (error) {
